...
export default function renderVideos(videoArray) {
  ...
  statisticsResponse.then((response) => {
    for (let i = 0; i < videoArray.length; i += 1) {
      ...
      author.innerHTML = `Channel: `;
      let span = addNewElement('span', 'meta-info_fetched-data', author);
      span.innerHTML = `${videoArray[i].snippet.channelTitle}`;
      ...
      span = addNewElement('span', 'meta-info_fetched-data', date);
      span.innerHTML = `${new Date(videoArray[i].snippet.publishedAt).toLocaleDateString('en-US', dateOptions)}`;
      const views = addNewElement('li', 'main__video-preview__video__meta-info_views', metaInfo);
      views.innerHTML = `Views: `;
      span = addNewElement('span', 'meta-info_fetched-data', views);
      span.innerHTML = `${response.items[i].statistics.viewCount}`;
